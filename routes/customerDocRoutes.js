...
router.get('/getAccountOwners', async (req, res) => {
    ...
    try {
        conn = await db.getConnection();
        const rows = await conn.query(`SELECT DISTINCT Account_Owner FROM customer`);
        res.json(rows);
    } catch (error) {
...
router.get('/getDocumentTypes', async (req, res) => {
    ...
    try {
        conn = await db.getConnection();
        const rows = await conn.query("SELECT Column_Description FROM final_module WHERE Column_Name = 'Document Type'");
        res.json(rows);
    } catch (error) {
...
router.get('/getServiceType', async (req, res) => {
    ...
    try {
        conn = await db.getConnection();
        const rows = await conn.query("SELECT Column_Description FROM final_module WHERE Column_Name = 'Service Type'");
        res.json(rows);
    } catch (error) {
        console.error('Error fetching document types:', error);
        res.status(500).json({ error: 'Database error' });
...
router.delete('/delete/:id', async(req, res) => {
    ...
    try {
        conn = await db.getConnection();
        const result = await conn.query('DELETE FROM customer_documents WHERE CD_ID = ?', [docID]);
